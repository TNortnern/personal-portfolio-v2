<template>
  <SectionBlock caption="Portfolio" title="My Works">
    <div class="flex justify-center text-lg gap-6 mt-16">
      <button
        v-for="tab in tabs" :key="tab" class="rounded-full py-2.5 w-30 duration-150 capitalize"
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-4 gap-y-8 justify-items-center pt-16">
      <router-link v-for="project of all" :key="project.id" :to="`/projects/${project.id}`" class="bg-white shadow-2xl group relative rounded-md overflow-hidden w-full">
